...
class BasicMetric:
    ...
    def eval(self, graph_dataset):
        ...
        if graph_dataset.round == 0:
            ...
            if self.user_record['liked_quality_contribution'][-1].numel() == 0:
                self.output['Metric-Inter-User-Like-Quality_Contribution'].append(0.0)
            else:
            ...
            if self.user_record['liked_match_contribution'][-1].numel() == 0:
                self.output['Metric-Inter-User-Like-Match_Contribution'].append(0.0)
            else:
        ...
        self.output['Metric-Quality-Exposure-Weighted'].append(
            self.weighted_mean(self.article_record['quality'][-1], self.article_record['exposure'][-1])
        )
        self.output['Metric-Quality-Click-Weighted'].append(
            self.weighted_mean(self.article_record['quality'][-1], self.article_record['click'][-1])
        )
        self.output['Metric-Quality-Like-Weighted'].append(
            self.weighted_mean(self.article_record['quality'][-1], self.article_record['like'][-1])
        )
        if graph_dataset.round == 0:
            ...
            self.output['Metric-Quality-Exposure-Corrcoef'].append(
                self.corrcoef(self.article_record['quality'][-1], self.article_record['exposure'][-1])
            )
            self.output['Metric-Quality-Click-Corrcoef'].append(
                self.corrcoef(self.article_record['quality'][-1], self.article_record['click'][-1])
            )
            self.output['Metric-Quality-Like-Corrcoef'].append(
                self.corrcoef(self.article_record['quality'][-1], self.article_record['like'][-1])
            )
            # 1.4 Recsys Infomation
        ...
        # 1.3 Homogenization
        sample_size = max(1, int(self._config['num_user']/10))
        sample_index = np.random.choice(list(range(self._config['num_user'])), sample_size, replace=False)
        ...
        like_r_eid_this_round = self.filter_edges_by_round(sg_recent_N_round_article, edges_this_round, 'like_r')
        click_r_eid_this_round = self.filter_edges_by_round(sg_recent_N_round_article, edges_this_round, 'click_r')
        exposure_r_eid_this_round = self.filter_edges_by_round(sg_recent_N_round_article, edges_this_round, 'exposure_r')
        ...
        self.output['Latent-User_Article_Similarity-Exposure'].append(
            self.user_article_similarity(sg_this_round, 'exposure_r')
        )
        self.output['Latent-User_Article_Similarity-Click'].append(
            self.user_article_similarity(sg_this_round, 'click_r')
        )
        self.output['Latent-User_Article_Similarity-Like'].append(
            self.user_article_similarity(sg_this_round, 'like_r')
        )

        self.output['Latent-In_List_Article_Similarity-Exposure'].append(
            self.inlist_article_similarity(sg_this_round, 'exposure_r')
        )
        self.output['Latent-In_List_Article_Similarity-Click'].append(
            self.inlist_article_similarity(sg_this_round, 'click_r')
        )
        self.output['Latent-In_List_Article_Similarity-Like'].append(
            self.inlist_article_similarity(sg_this_round, 'like_r', weighted=True)
        )
        ...
        if graph_dataset.round == 0:
            ...
            self.output['Latent-Cross_Round_Article_Shifting-Exposure'].append(
                self.cross_round_article_shifting(sg_recent_N_round_article, 'exposure_r')
            )
            self.output['Latent-Cross_Round_Article_Shifting-Click'].append(
                self.cross_round_article_shifting(sg_recent_N_round_article, 'click_r')
            )
            self.output['Latent-Cross_Round_Article_Shifting-Like'].append(
                self.cross_round_article_shifting(sg_recent_N_round_article, 'like_r', weighted=True)
            )
    ...
    def filter_edges_by_round(self, graph, predicate, etype):
        if graph.num_edges(etype) == 0 or 'round' not in graph.edges[etype].data:
            return torch.tensor([], dtype=torch.int32)
        return graph.filter_edges(predicate, etype=etype)

    def weighted_mean(self, values, weights):
        weights_sum = weights.float().sum()
        if weights_sum <= EPSILONE:
            return 0.0
        return float((values.float() * weights.float() / weights_sum).sum())
    ...
    def corrcoef(self, x, y):
        ...
        x = np.asarray(x)
        y = np.asarray(y)
        if x.size < 2 or y.size < 2 or np.std(x) <= EPSILONE or np.std(y) <= EPSILONE:
            return 0.0
    ...
    def user_article_similarity(self, graph, etype):
        if graph.num_edges(etype) == 0:
            return 0.0
        graph.update_all(msg_user_article_similarity, reduce_user_article_similarity, etype=('article', etype, 'user'))
        degrees = graph.in_degrees(etype=etype).float()
        degrees_sum = degrees.sum()
        if degrees_sum <= EPSILONE:
            return 0.0
        return float((graph.ndata['user_article_mean_similarity']['user'] * degrees / degrees_sum).sum())

    def inlist_article_similarity(self, graph, etype, weighted=False):
        if graph.num_edges(etype) == 0:
            return 0.0
        graph.update_all(msg_inlist_article_similarity, reduce_inlist_article_similarity, etype=('article', etype, 'user'))
        values = graph.ndata['inlist_article_similarity']['user']
        if not weighted:
            return float(values.mean())
        weights = graph.ndata['inlist_article_similarity_weight']['user'].float()
        weights_sum = weights.sum()
        if weights_sum <= EPSILONE:
            return 0.0
        return float((values * weights / weights_sum).sum())

    def cross_round_article_shifting(self, graph, etype, weighted=False):
        if graph.num_edges(etype) == 0:
            return 0.0
        graph.update_all(msg_cross_round_article_shifting, reduce_cross_round_article_shifting, etype=('article', etype, 'user'))
        values = graph.ndata['cross_round_article_shifting']['user']
        if not weighted:
            return float(values.mean())
        weights = graph.ndata['cross_round_article_shifting_weight']['user'].float()
        weights_sum = weights.sum()
        if weights_sum <= EPSILONE:
            return 0.0
        return float((values * weights / weights_sum).sum())
    ...
    def gini(self, x):
        ...
        if sum_x <= EPSILONE:
            return 0.0
        xarray = np.array(range(0, len(cum_x))) / float(len(cum_x) - 1)
        yarray = cum_x / sum_x
