...
class SimuLineGraphDataset(DGLDataset):
    ...
    def process(self):
        ...
        if user_latent.shape[0] < self._num_node['user']:
            raise ValueError('num_user exceeds the available preprocessed user samples')
        if creator_latent.shape[0] < self._num_node['creator']:
            raise ValueError('num_creator exceeds the available preprocessed creator samples')
        user_latent = user_latent[:self._num_node['user']]
        creator_latent = creator_latent[:self._num_node['creator']]
    ...
    def interaction_records(self):
        ...
        # query data
        like_eid = self.filter_edges_by_round(sg_recent_N_round_article, edges_in_prev_N_round, 'like')
        click_eid = self.filter_edges_by_round(sg_recent_N_round_article, edges_in_prev_N_round, 'click')
        exposure_eid = self.filter_edges_by_round(sg_recent_N_round_article, edges_in_prev_N_round, 'exposure')
        like_round = self.edge_rounds(sg_recent_N_round_article, 'like', like_eid)
        click_round = self.edge_rounds(sg_recent_N_round_article, 'click', click_eid)
        exposure_round = self.edge_rounds(sg_recent_N_round_article, 'exposure', exposure_eid)
    ...
    def edge_rounds(self, graph, etype, eids):
        if eids.shape[0] == 0 or 'round' not in graph.edges[etype].data:
            return torch.tensor([], dtype=torch.int32)
        return graph.edges[etype].data['round'][eids]
