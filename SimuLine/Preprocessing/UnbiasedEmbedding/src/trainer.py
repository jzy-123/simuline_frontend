...
def train_pointwise(sess: tf.version, model: PointwiseRecommender, data: str,
...
def train_pairwise(sess: tf.version, model: PairwiseRecommender, data: str,
...
class Trainer:
    ...
    def run(self, num_sims: int = 10) -> None:
        ...
        num_users = max(max(int(train_point[:, 0].max() + 1), int(test_point[:, 0].max() + 1)), int(val_point[:, 0].max() + 1))
        num_items = max(max(int(train_point[:, 1].max() + 1), int(test_point[:, 1].max() + 1)), int(val_point[:, 1].max() + 1))
        ...
        pd.concat(result_list, axis=1).to_csv(ret_path / 'aoa_all.csv')
        pd.concat(cold_user_result_list, axis=1).to_csv(ret_path / 'aoa_cold-user.csv')
        pd.concat(rare_item_result_list, axis=1).to_csv(ret_path / 'aoa_rare-item.csv')
