...
class PointwiseRecommender(AbstractRecommender):
    ...
    def build_graph(self) -> None:
        ...
        with tf.name_scope('embedding_layer'):
            self.user_embeddings = tf.compat.v1.get_variable('user_embeddings', shape=[self.num_users, self.dim],
                                                   initializer=tf.contrib.layers.xavier_initializer())
            if len(self.pretrained_item_embeddings.shape) == 1:
                self.item_embeddings = tf.compat.v1.get_variable('item_embeddings', shape=[self.num_items, self.dim],
                                                    initializer=tf.contrib.layers.xavier_initializer())
            else:
                self.item_embeddings = tf.compat.v1.get_variable("item_embeddings", shape=[self.num_items, self.dim], initializer=tf.constant_initializer(self.pretrained_item_embeddings), trainable=False)
...
class PairwiseRecommender(AbstractRecommender):
    ...
    def create_placeholders(self) -> None:
        """Create the placeholders to be used."""
        self.users = tf.compat.v1.placeholder(tf.int32, [None], name='user_ph1')
        self.pos_items = tf.compat.v1.placeholder(tf.int32, [None], name='item_ph1')
        self.scores1 = tf.compat.v1.placeholder(tf.float32, [None, 1], name='score_ph')
        self.items2 = tf.compat.v1.placeholder(tf.int32, [None], name='item_ph2')
        self.scores2 = tf.compat.v1.placeholder(tf.float32, [None, 1], name='score_ph')
        self.labels2 = tf.compat.v1.placeholder(tf.float32, [None, 1], name='label_ph2')
        self.rel1 = tf.compat.v1.placeholder(tf.float32, [None, 1], name='rel_ph1')
        self.rel2 = tf.compat.v1.placeholder(tf.float32, [None, 1], name='rel_ph2')
    ...
    def create_losses(self) -> None:
        """Create the losses."""
        with tf.name_scope('losses'):
            # 避免 preds 出现 0 时 log 变成 -inf
            preds_safe = tf.clip_by_value(self.preds, 1e-10, 1.0)

            # define the naive pairwise loss.
            local_losses = - self.rel1 * (1 - self.rel2) * tf.math.log(preds_safe)
            self.ideal_loss = tf.reduce_sum(local_losses) / tf.reduce_sum(self.rel1 * (1 - self.rel2))

            # define the unbiased pairwise loss.
            local_losses = - (1 / self.scores1) * (1 - (self.labels2 / self.scores2)) * tf.math.log(preds_safe)
